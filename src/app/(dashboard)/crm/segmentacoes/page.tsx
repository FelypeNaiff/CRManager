...
export default function SegmentacoesPage() {
  ...
  const [filterMesAniversario, setFilterMesAniversario] = useState("")
  const [filterMesAniversarioFilho, setFilterMesAniversarioFilho] = useState("")
  const [filterKidSexo, setFilterKidSexo] = useState("")
  ...
  const [filterVip, setFilterVip] = useState("all") // all, yes, no
  const [filterInatividade, setFilterInatividade] = useState("all") // all, 30, 60, 90
  ...
  const { data: tags } = useCollection(tagsQuery)

  // Join client data with financial statistics, children and tags
  const clientStats = useMemo(() => {
    ...
    if (!clientes) return stats

    // Initialize
    clientes.forEach((c: any) => {
      const wallet = carteiras?.find((w: any) => w.cliente_id === c.id)
      const kids = filhos?.filter((f: any) => f.cliente_id === c.id) || []
      
      stats[c.id] = {
        totalComprado: 0,
        ticketMedio: 0,
        ultimaCompra: null,
        saldoDisponivel: wallet?.saldo_atual || 0,
        filhos: kids,
        tags: c.tags || []
      }
    })
    ...
    if (vendas) {
      vendas.forEach((v: any) => {
        if (stats[v.clientId]) {
          ...
          stats[v.clientId].totalComprado += amt
          
          if (v.dataVenda?.seconds) {
            const dateStr = format(new Date(v.dataVenda.seconds * 1000), "yyyy-MM-dd")
            if (!stats[v.clientId].ultimaCompra || dateStr > stats[v.clientId].ultimaCompra!) {
              stats[v.clientId].ultimaCompra = dateStr
            }
          }
        }
      })
    ...
    clientes.forEach((c: any) => {
      if (stats[c.id]) {
        const clientSales = vendas?.filter((v: any) => v.clientId === c.id) || []
        if (clientSales.length > 0) {
          stats[c.id].ticketMedio = stats[c.id].totalComprado / clientSales.length
        }
      }
    })

    return stats
  }, [clientes, filhos, carteiras, vendas])
  ...
  const filteredClients = useMemo(() => {
    ...
    return clientes.filter((c: any) => {
      // 1. WhatsApp & Opt-in validation
      const cleanWhats = (c.whatsapp_principal || c.whatsapp || "").replace(/\D/g, "")
      if (cleanWhats.length < 10) return false // skip invalid contacts
      
      // Mandatory rules: respect opt-out!
      ...
      if (
        ...
        const hasMatchingKid = stat.filhos.some((k: any) => {
          if (filterKidSexo && k.sexo !== filterKidSexo) return false
  ...
  const handleClearFilters = () => {
    ...
    setFilterMesAniversario("")
    setFilterMesAniversarioFilho("")
    setFilterKidSexo("")
  ...
  // Toggle selected tags in filter
  ...
  const handleExportCsv = () => {
    ...
    const rows = filteredClients.map((c: any) => {
      ...
      const kidsList = statsObj?.filhos.map(k => `${k.nome} (${k.tamanho_roupa || 'N/A'})`).join(" | ") || ""
      return [
        c.nome,
        c.whatsapp_principal || c.whatsapp,
        c.vip ? "SIM" : "NAO",
        `R$ ${statsObj?.saldoDisponivel.toFixed(2)}`,
        kidsList,
        `R$ ${statsObj?.ticketMedio.toFixed(2)}`
      ]
    })
  ...
  // Send segments info to Campaign Page
  const handleCreateCampaign = () => {
    ...
    // Persist segment state in localStorage for campaigns page loading
  ...
  return (
    <div className="space-y-6 max-w-full overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <Card className="border shadow-sm bg-white">
            ...
            <CardContent className="space-y-4 pt-4 text-xs">
              ...
              <div className="border-t border-slate-100 pt-3 space-y-3">
                <span className="font-bold text-[10px] text-slate-400 uppercase tracking-wider block flex items-center gap-1"><Baby className="h-3.5 w-3.5 text-pink-500" /> Filtros do Filho</span>
                  ...
                  {tags?.map(t => {
                    ...
                    return (
                      <Badge 
                        ...
                        style={{ backgroundColor: isSelected ? t.cor || "#4f46e5" : "#e2e8f0", color: isSelected ? "white" : "#475569" }}
        ...
        {/* RIGHT COLUMN: REAL-TIME COUNTER & SEGMENT ELIGIBLE LIST */}
