...
import { getAvailableProfiles, logoutSession, validateProfilePin } from "@/lib/auth/actions"

interface ProfileSelectionClientProps {
  authenticatedEmail: string
}

export function ProfileSelectionClient({ authenticatedEmail }: ProfileSelectionClientProps) {
  const router = useRouter()
  const { loginProfile, logoutProfile } = useProfile()
    ...
    <div className="min-h-screen flex items-center justify-center bg-muted/30 p-4">
      <div className="max-w-4xl w-full space-y-8">
        <div className="text-center space-y-2">
          ...
          <p className="text-slate-500">Logado como <strong className="text-slate-700">{authenticatedEmail}</strong></p>
        </div>
