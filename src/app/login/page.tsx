  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (!ALLOWED_EMAILS.includes(user.email)) {
        await signOut(auth)
        throw new Error('Este email não está autorizado a acessar o CRM TRUPE.')
      }
        ...
        <CardContent>
          <div className="grid gap-4">
            <div className="text-center text-sm text-muted-foreground mb-2">
              Acesso restrito. Utilize uma conta corporativa autorizada para continuar.
            </div>
            
            <Button 
              variant="outline"
              className="w-full text-base h-12 gap-3 mt-2 font-medium bg-white hover:bg-slate-50 text-slate-700 border-slate-300" 
